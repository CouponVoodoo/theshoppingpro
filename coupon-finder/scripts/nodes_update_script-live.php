<?PHP
	...
	$Query = db_select('1Variables', 'b')
				 ...
				 ->condition('b.Variable', 'Update_Node_Script', '=');
	...
	mail('team@theshoppingpro.com', 'Update Node Script Start', gmdate('Y-m-d\TH:i:s\Z', (time()+(5.5*3600))));
		...
		$myntra_count = db_query("SELECT count(*) FROM  coupon_finder.predictorCompiledResultTable WHERE RetailerId = 8")->fetchField();
		$jabong_count = db_query("SELECT count(*) FROM  coupon_finder.predictorCompiledResultTable WHERE RetailerId = 5")->fetchField();
		$flipkart_count = db_query("SELECT count(*) FROM  coupon_finder.predictorCompiledResultTable WHERE RetailerId = 13419")->fetchField();
		$firstcry_count = db_query("SELECT count(*) FROM  coupon_finder.predictorCompiledResultTable WHERE RetailerId = 3")->fetchField();
		$all_count = db_query("SELECT count(*) FROM  coupon_finder.predictorCompiledResultTable")->fetchField();
