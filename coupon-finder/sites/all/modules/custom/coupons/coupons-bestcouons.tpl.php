...
<div class="search_listing_right">
  <div class="search_listing_row__<?php print $row->counter; ?> copy_coupon_row">
	...
	?>
    <a href="<?php print $base_url ?>/coupon-redirect?l=bc&os=<?php print $add_tracking;?>&nid=<?php print $nid;?>&c=<?php print $row->couponCode; ?>&p=<?php print $url_path; ?>&s=<?php print urlencode($row->url);?>" target="_blank" class="unlock_best_coupon unlock_coupon" id="<?php echo 'bc_'.$row->counter.'_'.$row->couponCode;?>" rel="best_<?php print $row->counter; ?>" data-clipboard-text="<?php echo $row->couponCode?>" >
