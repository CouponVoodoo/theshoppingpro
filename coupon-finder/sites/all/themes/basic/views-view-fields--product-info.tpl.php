...
<?php
	...
	// if(!$is_mobile && !$is_tablet){
		...
		$full_data_array_std_class = db_query("SELECT updateDate, couponStatus, BestCouponCode, NetPriceAfterSaving FROM {priceHistory} WHERE entity_id = ".$nid." order by updateDate desc limit 35")->fetchAll();
...
<?php if ( $time_gap > (1.5 * 24 * 3600)) {?>
	<div class="custom_link_inner"><span style= "color: red; font-weight: bold"><?php echo get_label('Data is more than 24hrs old: ');?></span>
	</div>
<?php } 
...
<div class='blue_button'><a href="#All_Coupons" class='d_view_store'><?php echo 'Click To View All Tested Coupons For This Product'; ?></a></div>
<?php } else { ?>
	...
	<?php
		foreach($predictor_array as $predictor_set){
			...
			$i++; 
					...
					if ($successful_predictor=="1" || $successful_predictor=="2") {
			...
			<li id="search_listing_li_row_<?php echo $i;?>" class="search_listing_row_li first">
				<div class="search_listing_left">
				  ...
				  <div class="row_2">
					...
					<?php elseif ($successful_predictor=="2" || $successful_predictor==2) : ?>	
					    <label><?php echo get_label('Savings:');?></label>
					...
					<?php else: ?>
						<label><?php echo get_label('Savings:');?></label>
						<div class="search_listing_row_1 search_listing_row"><div class="saving"></div><?php var_dump($successful_predictor);print get_label('This coupon does not work for your product'); ?></div>
					<?php endif; ?>
				  </div>
...
<?php
		...
		$full_data_array_std_class = db_query("select * from (SELECT updateDate, couponStatus, BestCouponCode, NetPriceAfterSaving FROM {priceHistory} WHERE entity_id = ".$nid." order by updateDate desc limit 35) as a order by updateDate asc")->fetchAll();
		$full_data_array = json_decode(drupal_json_encode($full_data_array_std_class), true);
