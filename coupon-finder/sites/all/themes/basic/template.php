		...
		switch ($content_type) {
			...
			case "node":
				...
				if($node->type == '_product_and_coupon') {
					$node_title = $node->field_retailer_product_name['und']['0']['value'].' @ Rs. '.$node->field_best_coupon_netpriceafters['und']['0']['value'].' (CV'.$nid.') | CupoNation';
...
function basic_breadcrumb($variables) {
    ...
    if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {
        ...
        if (!empty($breadcrumb)) {
            ...
            return $heading . '<div class="breadcrumb" itemscope itemtype="http://data-vocabulary.org/Breadcrumb">' . implode($breadcrumb_separator.$breadcrumb_separator, $breadcrumb) . $trailing_separator . $title . '</li></div>';
        }
    }
